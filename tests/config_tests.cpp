...
const std::vector<std::pair<std::string_view, std::string_view>>
  simple_config_expect = {
    ...
    std::make_pair("comment_after_code", "1234"),
    std::make_pair("empty_value", ""),
...
TEST_F(ConfigShould, DoNotCrashWhenNotOpened)
{
  Config<128> config("./data/boot2.conf");
  const auto bootType = config["BootType"];
  const auto bootPartition = config["BootPartition"];
  const auto imageType = config["ImageType"];

  EXPECT_THAT(*bootType, "");
  EXPECT_THAT(*imageType, "");
  EXPECT_THAT(*bootPartition, "");
}

TEST_F(ConfigShould, ReadSimpleConfigurationFileUnordered)
{
  Config<128> config("./data/boot.conf");
  const auto bootType = config["BootType"];
  const auto bootPartition = config["BootPartition"];
  const auto imageType = config["ImageType"];

  EXPECT_THAT(*bootType, "raw");
  EXPECT_THAT(*imageType, "raw");
  EXPECT_THAT(*bootPartition, "2");
}
